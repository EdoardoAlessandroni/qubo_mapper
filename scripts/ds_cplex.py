...
class Problem():
    ...
    def solve_exact(self):
        ...
        status, fval = result.get_status(), result.get_objective_value()
        return status, fval
    ...
    def solve_quantum(self, M_strategy):
        ...
        # write qubo and read it with cplex
        filename = self.qp.name + "_tmp"
        file = open(filename, "w")
        file.write(qubo.export_as_lp_string())
        file.close()

        model = cplex.Cplex(filename)
        #model.parameters.parallel.set(-1)
        model.parameters.threads.set(1)
        model.set_results_stream(None)
        model.set_log_stream(None)
        ...
        tac = time.time()
        result = model.solution
        x, fval = np.rint( result.get_values() ).astype(int), np.rint( result.get_objective_value() )

        # delete qubo tmp file
        os.remove(filename)
        return (x, fval), M, tac - tic
    ...
    def solve_unconstrained(self, how = "SDP"):
        ...
        if how == "SDP":
            ...
            p_unc = Problem(qp)
            filename = self.qp.name + "_tmp"
            p_unc.qp.name = filename
            p_unc.write_to_lp_file()
            _, fval = p_unc.solve_exact()
            os.remove(filename)
            return fval
        else:
