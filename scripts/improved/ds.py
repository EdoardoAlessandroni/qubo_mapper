...
class Problem():
    ...
    def improved_M(self):
        ...
        p = Problem(qp)
        result_unc = p.solve_exact()
        x_gl = np.rint( result_unc.x ).astype(int)

        # violation number
        A, b = self.get_constr_matrices()
        v = np.dot(A, x_gl) - b
        k = np.linalg.norm(v)**2
        print(f"Violation number: {k}")
        if k == 0:
            return 0
        ...
        grad = 2*v.T.dot(A)
        I = np.sum(grad < 0)
        improv = np.sqrt(I)*np.linalg.norm(grad)
        improv = np.linalg.norm(grad)
        print(improv)

        if k <= improv:
            return our_M
        print("CI SIAMO!")
        correction = np.ceil(k - improv)
        return our_M / correction
