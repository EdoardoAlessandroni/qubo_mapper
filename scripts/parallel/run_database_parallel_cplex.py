...
from ds_cplex import Datas, Problem, initial_hamiltonian_adiabatic
...
def run_instance(filename, M_strategies, indexes, analyze_gaps, analyze_gaps_qite, analyze_gaps_adiabatic): # data is after M_strategies
    '''
    ...
    for M_idx in range(len(M_strategies)):
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            (x, q_fval), M, t = p.solve_quantum(M_strategies[M_idx])
        """ data.fval[indexes[0], indexes[1], 0, M_idx] = np.rint(c_fval)
        data.fval[indexes[0], indexes[1], 1, M_idx] = q_fval
        ...
        if analyze_gaps:
            ...
            if analyze_gaps_qite:
                ...
                if analyze_gaps_adiabatic:
                    ...
                    #data.gap_minimal[indexes[0], indexes[1], M_idx] = gaps_various_alphas(21, H_0, H)
    ...
    """ if evaluate_feas:
        for M_idx in range(len(M_strategies)):
            evaluate_feasibility(p, xs[M_idx], data, indexes + [M_idx]) """
    print(f"exit{indexes[1]}")
    return p, xs
...
def run_test(test_set, bvars, n_samples, M_strategies, analyze_gaps, analyze_gaps_qite, analyze_gaps_adiabatic):
    '''
    Run simulation of problems (read from files) for differen
    t number of qubits, M-choice strategies, and samples and return data acquired
    '''
    ...
    for i in range(len(bvars)):
        ...
        """ for sample in range(n_samples):
            ...
            p, xs = run_instance(filename, M_strategies, [i, sample], analyze_gaps, analyze_gaps_qite, analyze_gaps_adiabatic)
            #for bigM_idx in range(n_M_strategies):
                #evaluate_feasibility(p, xs[bigM_idx], data, [i, sample, bigM_idx]) """
   
        
        pool = mp.Pool(1)        
        [pool.apply_async(run_instance, args = [folder + files[sample], M_strategies, [i, sample], analyze_gaps, analyze_gaps_qite, analyze_gaps_adiabatic]) 
                    for sample in range(n_samples)]
        #res = [w.get() for w in workers]
            #p = mp.Process(target = run_instance, args = (filename, analyze_gaps, M_strategies, data, [i, sample]))
            #p.start()
        pool.close()
        pool.join()
        tac = time()
...
test_set = "../toys/NN_linear"
analyze_gaps, analyze_gaps_qite, analyze_gaps_adiabatic = True, True, True
data = run_test(test_set, bvars, n_samples, M_strategies, analyze_gaps, analyze_gaps_qite, analyze_gaps_adiabatic)

# Save Datas()
""" file = open("../data/blabla.txt", "wb")
